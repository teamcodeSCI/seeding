...
class LeadEditModal {
    constructor({ data, closeLeadEditModal }) {
        ...
        this.$name = new InputGroup({ placeholder: "Họ và tên", width: '48%', value: this.data.name });
        this.$phone = new InputGroup({ placeholder: "Điện thoại", width: '48%', value: this.data.phonenumber });
        this.$nameFb = new InputGroup({ placeholder: "Tên FB", width: '48%', value: this.data.nameFb });
        this.$linkFb = new InputGroup({ placeholder: "Link FB", width: '48%', value: this.data.linkFb });
        this.$service = new InputGroup({ placeholder: "Dịch vụ đăng ký", width: '48%', value: this.data.service });
        this.$branch = new InputGroup({ placeholder: "Chi nhánh", width: '48%', value: this.data.branch });
        this.$script = new InputGroup({ placeholder: "Kịch bản", width: '48%', value: this.data.script });
        this.$interactive = new InputGroup({ placeholder: "Tương tác", width: '48%', value: this.data.interactive });
        this.$note = new Textarea({ palaceHolder: 'Ghi chú', value: this.data.note })
    }
    clickSave = () => {
