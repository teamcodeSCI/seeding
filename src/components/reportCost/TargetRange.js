...
class TargetRange {
    ...
    getTarget = async() => {
        const fetchUser = await getUser({
            userCode: role === "user" ?
                splitStr(localStorage.getItem("token")).code : this.user || "US0000015"
        });
        const user = fetchUser.data.pop();
        this.$targetText.innerHTML = user.kpi_target === 0 ? '0%' : `${(
          (user.kpi_now / user.kpi_target ) *
          100
        ).toFixed(1)}%`;
        this.targetDataSet[0].data = user.kpi_target === 0 ? [0, 1] : [user.kpi_now, user.kpi_target - user.kpi_now];
        this.$targetChart = new DoughnutChart({
            labels: this.targetLabels,
            dataSet: this.targetDataSet
        });
