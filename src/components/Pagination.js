class Pagination {
  constructor({ count, index, setIndex, getAllData }) {
    ...
    this.count =
      this.limit + this.index - 1 >= this.dataLength
        ? this.dataLength
        : this.limit + this.index - 1; // giới hạn phần tử lớn nhất được render của pagination
    this.$container = document.createElement("ul");
    this.$container.id = "page2";
    this.$container.className = "pagination justify-content-center m-0";

    //nút Về đầu trang
    this.$firstPageBtn = document.createElement("li");
    this.$firstPageBtn.className = `page-item ${
      this.index <= 1 || this.dataLength === 0 ? "d-none" : ""
    }`;
    ...
    this.$preBtn.className = `page-item ${
      this.index <= 1 || this.dataLength === 0 ? "d-none" : ""
    }`;
    ...
    this.$nextBtn.className = `page-item ${
      this.index >= this.dataLength ? "d-none" : ""
    }`;
    ...
    this.$endBtn.className = `page-item ${
      this.index >= this.dataLength ? "d-none" : ""
    }`;
  ...
  nextPage() {
    this.index =
      this.index >= this.limit + this.index - 1
        ? this.dataLength
        : this.index + 1;
    this.setIndex(this.index);
    this.getAllData();
  ...
  render() {
    ...
    const numTemp = (number) => {
      ...
      this.$numBtn.className = `page-item ${
        this.index === number ? "active" : ""
      }`;
