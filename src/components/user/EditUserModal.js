...
class EditUserModal {
  constructor({ closeUserEditModal, user, getAllUser }) {
    ...
    this.$retypePassword = new InputGroup({
  ...
  save = async () => {
    ...
    if (
      this.$password.getValue().value !== this.$retypePassword.getValue().value
    ) {
      this.$notify.innerHTML = "Mật khẩu không khớp!";
      return;
    }
    this.$notify.innerHTML = "";
    const update = await updatePassUser({
      user: this.user,
      password: this.$password.getValue().value
    });
    if (update.type !== 0) {
